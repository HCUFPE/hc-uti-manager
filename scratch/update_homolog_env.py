...
def main():
    ...
    if not host or not user or not secret:
        print("Erro: Credenciais de homologação não encontradas no .env local!")
        return

    print(f"Conectando à VM de Homologação {host} para atualizar o .env...")
    ...
    try:
        ...
        # Modificar o JWT_SECRET gerando um novo token seguro exclusivo para Homologação
        print("Gerando e atualizando JWT_SECRET...")
        ...
        print("Arquivo .env de homologação atualizado com sucesso!")
        sftp.close()
    except Exception as e:
        print("Erro ao atualizar o .env na VM de homologação:", e)
    finally:
        ssh.close()
