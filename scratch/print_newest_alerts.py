...
async def run():
    ...
    async for session in db.get_session():
        res = await session.execute(select(Alerta).order_by(Alerta.id.desc()).limit(15))
        alertas = res.scalars().all()
        for a in alertas:
            print(f"ID: {a.id} | Titulo: {a.titulo} | Lido: {a.lido} | Por: {a.lido_por} | Pront: {a.prontuario} | Msg: {a.mensagem[:100]}...")
